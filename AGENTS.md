...
### 🎯 專案目的
建立一個**Sportsnavi新聞爬蟲系統**，自動化收集、解析、儲存來自多個日本運動新聞網站的各種運動分類相關文章。

### 🏢 業務需求
- **多來源整合**：支援 Yahoo Sports 棒球分類下的多個新聞來源
  - 主站：https://sports.yahoo.co.jp
  - 棒球大分類：https://baseball.yahoo.co.jp/
  - 小分類（各 Agent）：
    - `/npb/` - 日本職棒（Nippon Professional Baseball）
    - `/mlb/` - 美國大聯盟（Major League Baseball）
    - `/hsb/` - 高校野球（High School Baseball）
    - `/bbl/` - 大學野球（University Baseball）
    - `/ind/` - 獨立聯盟（Independent League）
    - 其他分類...
- **定時爬取**：每天自動爬取 4 次（8:00, 12:00, 16:00, 20:00 台灣時區）
- **資料管理**：統一儲存、去重、分類、查詢
- **資料保留**：保留最近 13 個月資料，超過自動清理（每月 1 號凌晨 3:00）
- **資源限制**：系統記憶體限制 8GB RAM
- **Web 介面**：提供搜尋、瀏覽、匯出功能
...
以下反爬蟲對策暫時不實作，視網站限制情況再考慮：

- **Proxy Pool**：代理 IP 池輪換
- **CAPTCHA 處理**：驗證碼識別與處理
- **Rate Limiting**：動態請求頻率控制
- **Session 管理**：Cookie 和 Session 池
- **分散式爬取**：多節點部署

> 💡 **原因**：目前 Yahoo Sports 網站沒有嚴格的反爬蟲機制，基本的偽裝已足夠。如果未來遇到封鎖，再實作進階對策。
...
| 指標 | 數值 | 說明 |
|------|------|------|
| **爬蟲數量** | 6 個 | 棒球分類：NPB, MLB, HSB, BBL, IND, 業餘 |
| **定時頻率** | 4 次/天 | 8:00, 12:00, 16:00, 20:00 (台灣時區) |
| **平均文章數** | 15-20 篇/來源 | 每次爬取 |
| **成功率** | >90% | 有重試和容錯機制 |
| **執行時間** | 5-8 分鐘 | 並行模式（3 個並發），6 個來源 |
| **記憶體限制** | 8GB RAM | 系統總記憶體上限 |
| **並發數** | 3 個 | 最多 3 個爬蟲同時運行（防止記憶體溢出）|
| **資料保留** | 13 個月 | 超過自動清理（每月 1 號凌晨 3:00）|
| **資料庫大小** | ~6-8GB | 13 個月資料（包含全文內容）|
...
### 已完成 ✅
- [x] **記憶體管理**：8GB RAM 限制，並發控制（3 個）
- [x] **資料保留策略**：13 個月自動清理機制
- [x] **Chrome 記憶體優化**：無頭模式、禁用圖片載入
...
### 專案文檔
- **[記憶體優化配置](memory_optimization.md)** - 8GB RAM 限制下的並發控制與優化
- **[資料保留策略](data_retention_policy.md)** - 13 個月自動清理機制
...
**最後更新**：2025-11-03
**作者**：Claude Code
**專案**：Sportsnavi 新聞爬蟲系統

**v1.1 更新內容**：
- ✅ 明確網站架構（sports.yahoo.co.jp → baseball.yahoo.co.jp → 各分類）
- ✅ 修正命名（nplCrawler → NPBCrawler）
- ✅ 新增記憶體管理配置（8GB RAM 限制）
- ✅ 新增資料保留策略（13 個月）
- ✅ 更新關鍵指標（並發數、執行時間、資料庫大小）
- ✅ 標註反爬蟲對策實作狀態
